...
function Bartender2Fu:LockButtons()
        if LOCK_ACTIONBAR == "1" then
                LOCK_ACTIONBAR = "0"
                Bartender:Print(L["ActionBar lock |cffffffcf[|r|cffff0000Off|cffffffcf]|r"])
        else
                LOCK_ACTIONBAR = "1"
                Bartender:Print(L["ActionBar lock |cffffffcf[|r|cff00ff00On|cffffffcf]|r"])
        end
end
...
function Bartender2Fu:OnTooltipUpdate()
	...
	if LOCK_ACTIONBAR == "1" then
		cat:AddLine(	'text', L["Buttons:"],
								 	'text2', L["Locked"],
								 	'text2R', 0,
								 	'text2G', 1,
									'text2B', 0)
	else
		cat:AddLine(	'text', L["Status:"],
								 	'text2', L["Unlocked"],
								 	'text2R', 1,
								 	'text2G', 0,
									'text2B', 0)
	end
	Tablet:SetHint(L["Left-click to Lock/Unlock.\nRight-click to configure."])
end