...
function GCInFu:OnTextUpdate()
	if self.previousMem then
		if self.db.profile.Type == 1 then
			self.str = string.format(L"Cur: [%.1fMB]", self.currentMem/1024)
		elseif self.db.profile.Type == 2 then
			self.str = string.format(L"TH: [%.1fMB]", self.gcThreshold/1024)
		elseif self.db.profile.Type == 3 then
			self.str = string.format(L"Inc: [%.1fKB/s]", self.currentMem - self.previousMem)
		end
...
function GCInFu:OnTooltipUpdate()
	...
	self.tablet:SetHint(L"Shift+Ctrl-Click to force garbage collection.\nHint: Click tooltip to select visible InFu.")
