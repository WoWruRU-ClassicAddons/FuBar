local L = AceLibrary("AceLocale-2.0"):new("ModMenuTuFuDB")
...
local function ValidateCategory(i, cat)
	if cat == L["Unknown"] then cat = nil end
	return cat == L["All Mods"] or cat == GetAddOnMetadata(i, "X-Category")
end
...
function FuBar_ModMenuTuFu:OnMenuRequest(level, v1, intip, v2, v3, v4)
	if level == 1 then
		dewdrop:AddLine("text", L["By Category"], "value", L["By Category"], "hasArrow", true)
		dewdrop:AddLine("text", L["By Author"], "value", L["By Author"], "hasArrow", true)
		dewdrop:AddLine("text", L["By Dependancy"], "value", L["By Dependancy"], "hasArrow", true)
		dewdrop:AddLine("text", L["FuBar Options"], "value", L["FuBar Options"], "hasArrow", true)
	elseif level == 2 then
		if v1 == L["By Category"] then
			for _,i in ipairs(sortcategories) do dewdrop:AddLine("text", i, "value", i, "hasArrow", true) end
			dewdrop:AddLine()
			dewdrop:AddLine("text", L["Unknown"], "value", L["Unknown"], "hasArrow", true)
			dewdrop:AddLine()
			dewdrop:AddLine("text", L["All Mods"], "value", L["All Mods"], "hasArrow", true)
		elseif v1 == L["By Author"] then
			for _,i in ipairs(sortauthors) do dewdrop:AddLine("text", i, "value", i, "hasArrow", true) end
		elseif v1 == L["By Dependancy"] then
			for _,i in ipairs(sorttags) do dewdrop:AddLine("text", i, "value", i, "hasArrow", true) end
		elseif v1 == L["FuBar Options"] then self:AddImpliedMenuOptions(2) end
	elseif self:AddImpliedMenuOptions(2) then
	elseif level == 3 then
		if v2 == L["By Category"] then self:BuildAddonMenu(v1, ValidateCategory)
		elseif v2 == L["By Author"] then self:BuildAddonMenu(v1, ValidateAuthor)
		elseif v2 == L["By Dependancy"] then self:BuildAddonMenu(v1, ValidateDependant) end
	else
