...
function SkillsPlusFu:UpdateText(newProfession,newIcon)
	...
	if (totalItems > 0) then
		...
		cooldownInfo = format(colorCode.." (%d/%d)"..FONT_COLOR_CODE_CLOSE, readyItems, totalItems)
	end
				...
				for _,skill in category.skills do
					if category.category ~= L["COOLDOWN_CATEGORY"] then
						...
						if (timeRemaining <= 0) then
							timeRemainingText = '|cff00FF00'..L["COOLDOWN_READY"]
						else
							...
							if ((timeTable.d == 0) and (timeTable.h == 0)) then
								timeRemainingText = '|cffF0F000'..timeString
							else
								timeRemainingText = '|cffFF0000'..timeString
							end
						end
