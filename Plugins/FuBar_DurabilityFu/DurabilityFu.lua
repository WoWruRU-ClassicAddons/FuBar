...
DurabilityFu.version = "2.0." .. string.sub("$Revision: 14741 $", 12, -3)
DurabilityFu.date = string.sub("$Date: 2006-10-21 21:37:11 -0400 (Sat, 21 Oct 2006) $", 8, 17)
...
DurabilityFu:RegisterChatCommand({"/durfu", "/durabilityfu"}, options)
	...
	if CanMerchantRepair() then
		if self:IsAutoRepairingEquipment() then
			...
			if money < repairCost then
				self:Print(L["Cannot auto-repair equipment. Your money: %s. Needed: %s"], Abacus:FormatMoneyFull(money, true), Abacus:FormatMoneyFull(repairCost, true))
				stop = true
			elseif repairCost > 0 then
				RepairAllItems()
				self:Print(L["Auto-repaired equipment: %s"], Abacus:FormatMoneyFull(repairCost))
				dirty = true
			else
				self:Print(L["No equipment to auto-repair"])
			end
		end
		if not stop and self:IsAutoRepairingInventory() then
			...
			local money = GetMoney()
			if money < repairCost then
				self:Print(L["Cannot auto-repair inventory. Your money: %s. Needed: %s"], Abacus:FormatMoneyFull(money, true), Abacus:FormatMoneyFull(repairCost, true))
			elseif repairCost > 0 then
				ShowRepairCursor()
				for bag = 0, 4 do
					for slot = 1, GetContainerNumSlots(bag) do
				...
				HideRepairCursor()
				self:Print(L["Auto-repaired inventory: %s"], Abacus:FormatMoneyFull(repairCost))
				dirty = true
			else
				self:Print(L["No items in inventory to auto-repair"])
			end
		end
	end
...
function DurabilityFu:OnTextUpdate()
	...
	if not self:IsShowingAverage() then
		...
		local value = 0
		local max = 0
		for i,item in pairs(self.itemStatus) do
			if i ~= 12 then
...
function DurabilityFu:OnTooltipUpdate()
	...
	for index, item in pairs(self.itemStatus) do
		if item.max > 0 then
			cost = cost + item.cost
