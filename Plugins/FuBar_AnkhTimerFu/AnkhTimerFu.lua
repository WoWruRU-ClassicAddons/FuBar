...
AnkhTimerFu.date = string.sub("$Date: 2006-09-23 05:19:32 -0400 (Sat, 23 Sep 2006) $", 8, 17)
...
function AnkhTimerFu:OnInitialize()
    self.optionsTable = {
        ...
        args = {
            ankhcheck = {
                name = L["Ankh Count Check"], type = "range",
                desc = L["Minimum ankhs in inventory. If there are less, a warning popup will be displayed.\n'0' = disable"],
...
function AnkhTimerFu:OnEnable()
    ...
    StaticPopupDialogs["ANKHTIMERFU_POPUP"] = {
        text = L["You have only "]..self.db.profile.ankhCount..L[" Ankh(s) left!\n(check at "]..self.db.profile.ankhCheckNumber..")",
        button1 = TEXT(ACCEPT),
        showAlert = 1,
        timeout = 0,
        hideOnEscape = 1
...
function AnkhTimerFu:OnTextUpdate()
    local s = ""
    s = s .. format(L["|cff%s%d|r Ankh(s)"], crayon:GetThresholdHexColor(self.db.profile.ankhCount, 0, 1, 3, 5, 10), self.db.profile.ankhCount)
...
function AnkhTimerFu:OnTooltipUpdate()
    ...
    cat:AddLine(
        'text', L["Ability cooldown:"],
        'text2', (self:GetTalentCooldown()/60)..L[" min"]
    )
end
