...
local L = AceLibrary("AceLocale-2.0"):new("ItemRackFu")
...
function ItemRackFu:OnInitialize()
	self.options = {
		...
		args = {
			text = {
				type = "execute",
				name = L"Open ItemRack Settings",
				desc = L"Open ItemRack Settings Dialog",
				func = function()
...
function ItemRackFu:OnTooltipUpdate()
	if IR_outdated then
		Tablet:SetHint(L"ItemRack v1.83+ required.")
	else
		Tablet:SetHint(L"Left-click to choose a set.")
	end
end
...
function ItemRackFu:OnClick()
	if IR_outdated then
		self:Print(L"ItemRack version 1.83 or greater is required to open sets from FuBar.")
	else
