...
function CombatTimeFu:OnTextUpdate()
	if self.inCombat == true then
		self:SetText(L"Combat: ".."|cffff0000".. self:GetCurrentCombatTime() .."|r")
	else
		if self.combatEnd == 0 then
			self:SetText(L"Combat: "..L"|cff00ff00No|r")
		else
			self:SetText(L"Combat: ".."|cff00ff00".. self:GetCurrentCombatTime() .."|r")
		end
	end
end
...
function CombatTimeFu:OnTooltipUpdate()
	...
	cat:AddLine(
		'text', L"Combat status",
		'text2', self.inCombat and L"|cffff0000In combat|r" or L"|cff00ff00Not in combat|r"
	)
	cat:AddLine(
		'text', L"Combat time",
		'text2', self.combatStart > 0 and self:GetCurrentCombatTime() or L"None"
	)
end